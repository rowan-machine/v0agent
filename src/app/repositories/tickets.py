...
Adapters: SupabaseTicketRepository, SQLiteTicketRepository
"""
...
class SQLiteTicketRepository(TicketRepository):
    """
    SQLite adapter for ticket repository.
    """
    ...
    def _format_row(self, row) -> Dict[str, Any]:
        """Format SQLite row to standard ticket dict."""
        return {
            "id": row["id"],
            "ticket_id": row.get("ticket_id"),
            "title": row["title"],
    ...
    def get_by_id(self, entity_id: str) -> Optional[Dict[str, Any]]:
        ...
        with self._get_connection() as conn:
            # Try by id first
            row = conn.execute(
                "SELECT * FROM tickets WHERE id = ?", (entity_id,)
            ).fetchone()
            
            if not row:
                # Try by ticket_id
                row = conn.execute(
                    "SELECT * FROM tickets WHERE ticket_id = ?", (entity_id,)
                ).fetchone()
    ...
    def get_count(self, filters: Optional[Dict[str, Any]] = None) -> int:
        ...
        with self._get_connection() as conn:
            if filters and "statuses" in filters:
                ...
                result = conn.execute(
                    f"SELECT COUNT(*) FROM tickets WHERE status IN ({placeholders})",
                    filters["statuses"]
                ).fetchone()
            else:
    ...
    def create(self, data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        with self._get_connection() as conn:
            ...
            """, (
                ...
                data.get("priority"),
                data.get("sprint_points", 0),
                data.get("in_sprint", 1),
                data.get("tags"),
            ))
            conn.commit()
    ...
    def update(self, entity_id: str, data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        values = []
        allowed_fields = [
            "title", "description", "status", "priority", "sprint_points",
            "in_sprint", "ai_summary", "implementation_plan", "task_decomposition",
            "test_plan", "tags"
        ]
    ...
    def delete(self, entity_id: str) -> bool:
        """Delete a ticket."""
        with self._get_connection() as conn:
            conn.execute("DELETE FROM tickets WHERE id = ?", (entity_id,))
            conn.commit()
            return True
