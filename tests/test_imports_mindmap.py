...
def sample_meeting(mock_supabase_with_data):
    """Create a sample meeting for testing mindmap ingestion in mock Supabase."""
    mock_supabase_with_data.seed_data("meeting_summaries", [
        {
            "id": "uuid-meeting-mindmap",
            "meeting_name": "PBM Pipeline Planning Meeting",
            "synthesized_notes": "Discussion about Rx claim reallocation and DMP pipeline.",
    ...
    mock_supabase_with_data.seed_data("dikw_items", [])
    
    return "uuid-meeting-mindmap"
...
class TestMindmapIngestEndpoint:
    ...
    def test_successful_mindmap_upload(self, client_with_data, sample_meeting, sample_image_bytes, mock_vision_response):
    ...
    def test_extracts_entities(self, client_with_data, sample_meeting, sample_image_bytes, mock_vision_response):
    ...
    def test_extracts_relationships(self, client_with_data, sample_meeting, sample_image_bytes, mock_vision_response):
    ...
    def test_identifies_patterns(self, client_with_data, sample_meeting, sample_image_bytes, mock_vision_response):
    ...
    def test_generates_insights(self, client_with_data, sample_meeting, sample_image_bytes, mock_vision_response):
    ...
    def test_creates_dikw_items(self, client_with_data, sample_meeting, sample_image_bytes, mock_vision_response):
    ...
    def test_rejects_nonexistent_meeting(self, client, sample_image_bytes, mock_vision_response):
        """Should return 404 for non-existent meeting."""
        response = client.post(
            "/api/v1/imports/mindmap/nonexistent-uuid",
            files={"mindmap": ("mindmap.png", io.BytesIO(sample_image_bytes), "image/png")}
        )
        
        assert response.status_code == 404
    
    def test_rejects_unsupported_file_type(self, client_with_data, sample_meeting):
    ...
    def test_supports_jpeg_format(self, client_with_data, sample_meeting, mock_vision_response):
...
class TestListMindmapsEndpoint:
    ...
    def test_returns_empty_list_initially(self, client_with_data, sample_meeting):
        ...
        response = client_with_data.get(f"/api/v1/imports/meetings/{sample_meeting}/mindmaps")
        
        assert response.status_code == 200
        assert response.json() == []
    
    def test_returns_mindmaps_after_upload(self, client_with_data, sample_meeting, sample_image_bytes, mock_vision_response):
    ...
    def test_returns_404_for_nonexistent_meeting(self, client):
        """Should return 404 for non-existent meeting."""
        response = client.get("/api/v1/imports/meetings/nonexistent-uuid/mindmaps")
        assert response.status_code == 404
...
class TestDIKWIntegration:
    ...
    def test_creates_dikw_at_correct_levels(self, client_with_data, mock_supabase_with_data, sample_meeting, sample_image_bytes, mock_vision_response):
        ...
        # Check DIKW items were created via response
        assert response.status_code == 200
        assert response.json()["dikw_items_created"] >= 1
    
    def test_dikw_items_link_to_meeting(self, client_with_data, mock_supabase_with_data, sample_meeting, sample_image_bytes, mock_vision_response):
        ...
        assert response.status_code == 200
        # DIKW items are created and linked to meeting via Supabase
...
class TestMindmapEdgeCases:
    ...
    def test_handles_vision_api_failure(self, client_with_data, sample_meeting, sample_image_bytes):
    ...
    def test_handles_malformed_vision_response(self, client_with_data, sample_meeting, sample_image_bytes):
    ...
    def test_handles_empty_dikw_candidates(self, client_with_data, sample_meeting, sample_image_bytes):
